...
def txt_encode(text, password, cover_file_path, output_folder):
    l = len(text)
    i = 0
    add = ''
    while i < l:
...
def encode_txt_data():
    ...
    cover_file_path = filedialog.askopenfilename(title="Select an image to hide the message")
    count2 = 0
    ...
    if l <= bt:
        ...
        output_folder = filedialog.askdirectory(title="Select a folder to save the stego file")
        if output_folder:
            stego_file_path = txt_encode(text1, password, cover_file_path, output_folder)
            print("Stego file saved at:", stego_file_path)
        else:
...
def txt_decode(stego_file_path, password):
    ...
    while i < len(temp):
        ...
        if t3 == '0110':
            ...
            final += chr((decimal_data ^ 170) - 48)
    if password == "1234":  
        return final  # Return the decoded message
    else:
...
def decode_txt_data():
    ...
    password = input("Enter password: ")
    decoded_message = txt_decode(stego_file_path, password)
    print("\nMessage after decoding from the stego file:- ", decoded_message)
def txt_steg():
